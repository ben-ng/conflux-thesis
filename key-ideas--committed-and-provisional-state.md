...
Since Conflux has to be usable by developers without formal training in distributed systems, details like leader election and log replication cannot be exposed in the API. Instead, we introduce the idea of *provisional* and *committed* state.
...
Earlier we introduced methods, procedures that inspect the state of the application and may emit an action if the transition results in an externally consistent state. We now clarify that *methods inspect the leader's provisional state*.
